...
public:
  ...
  void write() {
    ...
    f << "Y_COORDINATES " << data[0].size() << " float" << std::endl;   // CHANGED data.size() -> data[0].size()
    for(unsigned i = 0; i < data[0].size(); ++i) {                      // CHANGED data.size() -> data[0].size()
      f << i * delta << std::endl;
    }
    ...
    f << "POINT_DATA " << (data.size() * data[0].size()) << std::endl; // CHANGED data.size() * data.size() -> data.size() * data[0].size()
